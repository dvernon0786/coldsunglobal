            ...
            {blogs.map(b => (
              <div key={b.title} className="card-lift p-8 rounded-xl border border-gray-100 bg-white group cursor-pointer">
                <div className="flex items-center gap-2 mb-4">
                  <span className="text-xs font-semibold px-2.5 py-1 rounded-full" style={{background:"var(--orange-pale)",color:"var(--orange)",fontFamily:"'DM Sans',sans-serif"}}>{b.category}</span>
                </div>
                <h3 className="font-bold text-base mb-3 leading-snug group-hover:text-orange-500 transition-colors" style={{color:"var(--navy)",fontFamily:"'Playfair Display',serif"}}>{b.title}</h3>
                <div className="flex items-center gap-3 text-xs text-gray-400 mt-4" style={{fontFamily:"'DM Sans',sans-serif"}}>
                  <span>{b.date}</span>
                  <span>·</span>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {ebooks.map(e => (
              <div key={e.title} className="card-lift p-8 rounded-xl bg-white border border-gray-100 flex gap-6 items-start">
                ...
                <div>
                  <span className="text-xs font-semibold" style={{color:"var(--orange)",fontFamily:"'DM Sans',sans-serif"}}>{e.tag}</span>
                  <h3 className="font-bold mt-1 mb-2" style={{color:"var(--navy)",fontFamily:"'Playfair Display',serif"}}>{e.title}</h3>
                  <p className="text-sm text-gray-500 mb-3" style={{fontFamily:"'DM Sans',sans-serif"}}>{e.desc}</p>
                  <Link href="/contact" className="text-sm font-semibold" style={{color:"var(--orange)",fontFamily:"'DM Sans',sans-serif"}}>Download Free →</Link>
                </div>
              </div>
            ))}
          </div>
            ...
            {webinars.map(w => (
              <div key={w.title} className="card-lift flex items-center justify-between p-6 rounded-xl border border-gray-100 bg-white">
                <div className="flex items-center gap-4">
                  <span className={`text-xs font-semibold px-3 py-1.5 rounded-full ${w.status === "Upcoming" ? "text-white" : "text-gray-600 bg-gray-100"}`} style={w.status === "Upcoming" ? {background:"var(--orange)",fontFamily:"'DM Sans',sans-serif"} : {fontFamily:"'DM Sans',sans-serif"}}>{w.status}</span>
                  <div>
                    <h3 className="font-bold" style={{color:"var(--navy)",fontFamily:"'Playfair Display',serif"}}>{w.title}</h3>
                    <p className="text-sm text-gray-400 mt-0.5" style={{fontFamily:"'DM Sans',sans-serif"}}>{w.date}</p>
                  </div>
                </div>
                <Link href="/contact" className="text-sm font-semibold flex items-center gap-1" style={{color:"var(--orange)",fontFamily:"'DM Sans',sans-serif"}}>
